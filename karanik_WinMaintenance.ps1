...
$Catalog = @(
  ...
  [pscustomobject]@{ Id="1.1";  Title="Update Windows";                                 Type="Remote"; ScriptName="UpdateWindows.ps1";        Sha256=$null; Group="Main";
    Tooltip="Scans for available Windows Updates and lists them (KB, severity, size, reboot required).`nPrompts to install with a 30-second countdown (default: Y).`n`nSteps:`n  1. Checks for PSWindowsUpdate module - installs from PSGallery if missing`n  2. Falls back to WUA COM API if PSWindowsUpdate is unavailable`n  3. Shows update table with severity color coding`n  4. Installs selected updates`n  5. Reports results and prompts for reboot if required" },
  [pscustomobject]@{ Id="1.2";  Title="Reset Windows Update";                           Type="Remote"; ScriptName="ResetWindowsUpdate.ps1";     Sha256=$null; Group="Main";
    Tooltip="Fully resets the Windows Update stack.`n`nSteps:`n  1. Stops services: wuauserv, bits, cryptsvc, msiserver, appidsvc, usosvc`n  2. Measures SoftwareDistribution + catroot2 disk usage (before)`n  3. Resets service permissions via sc sdset (bits, wuauserv)`n  4. Deletes SoftwareDistribution contents and stale .old/.bak backups`n  5. Clears catroot2 (System32 + SysNative if present)`n  6. Removes stale BITS queue files (qmgr*.dat)`n  7. Restarts all services in correct dependency order`n  8. Reports freed disk space and final service status`n`nUseful for: Windows Update stuck, 0x80... errors, corrupted WU database" },
  [pscustomobject]@{ Id="1.3";  Title="Defender: Update signatures & Quick Scan";       Type="Remote"; ScriptName="DefenderUpdate.ps1";          Sha256=$null; Group="Main";
    Tooltip="Updates Windows Defender antivirus signatures and runs a Quick Scan.`n`nSteps:`n  1. Update-MpSignature (falls back to MpCmdRun.exe -SignatureUpdate)`n  2. Start-MpScan -ScanType QuickScan (falls back to MpCmdRun.exe -Scan -ScanType 1)`n  3. Reports: signature version, last updated, real-time protection status" },
  [pscustomobject]@{ Id="1.4";  Title="GPUpdate /force (Computer + User)";              Type="Remote"; ScriptName="GPUpdate.ps1";                Sha256=$null; Group="Main";
    Tooltip="Runs gpupdate.exe /force to refresh all Group Policy settings immediately.`nApplies both Computer and User policies.`nOutput is logged line by line. May take up to 2 minutes on domain-joined machines." },
  [pscustomobject]@{ Id="1.5";  Title="Create System Restore Point";                    Type="Remote"; ScriptName="CreateRestorePoint.ps1";      Sha256=$null; Group="Main";
    Tooltip="Creates a Windows System Restore Point named 'karanik_WinMaintenance - <timestamp>'.`n`n- Checks if System Protection is enabled on C: and attempts to enable it if not`n- Uses Checkpoint-Computer with MODIFY_SETTINGS type`n- Reports: sequence number, description, creation time`n`nTip: Run before making system changes such as driver updates or major installs." },
  [pscustomobject]@{ Id="1.51"; Title="Clean All Restore Points";                       Type="Remote"; ScriptName="CleanRestorePoints.ps1";      Sha256=$null; Group="Main";
    Tooltip="Permanently deletes ALL System Restore Points on this machine.`n`nSteps:`n  1. Lists all existing restore points with sequence numbers and timestamps`n  2. Shows estimated shadow storage space in use`n  3. Prompts for confirmation (20-second countdown, default: Y)`n  4. Deletes via vssadmin delete shadows /all /quiet`n  5. Falls back to WMI (Win32_ShadowCopy) if vssadmin fails`n  6. Verifies and reports freed shadow storage`n`nWARNING: This action is irreversible. System rollback will not be possible after." },
  [pscustomobject]@{ Id="1.6";  Title="Clear Event Logs";                               Type="Remote"; ScriptName="ClearEventLogs.ps1";          Sha256=$null; Group="Main";
    Tooltip="Clears Windows Event Logs using wevtutil.exe (compatible with PS 5.1).`n`nLogs cleared:`n  - Application, System, Security, Setup`n  - Microsoft-Windows-WindowsUpdateClient/Operational`n  - Microsoft-Windows-Bits-Client/Operational`n  - Microsoft-Windows-Diagnostics-Performance/Operational`n`n- Skips logs that are already empty or inaccessible`n- Reports count of cleared vs skipped logs" },
  [pscustomobject]@{ Id="1.7";  Title="Install Winget (App Installer helper)";          Type="Remote"; ScriptName="InstallWinget.ps1";            Sha256=$null; Group="Main";
    Tooltip="Installs or updates the Windows Package Manager (winget).`n`nSteps:`n  1. Checks if winget.exe is already installed`n  2. Installs VCLibs x64 14.00 dependency (required by winget)`n  3. Fetches latest release from GitHub API (microsoft/winget-cli)`n  4. Downloads and installs the .msixbundle`n  5. Falls back to opening Microsoft Store if GitHub fails`n  6. Verifies installed version after install" },
  [pscustomobject]@{ Id="1.8";  Title="Install latest PowerShell";                      Type="Remote"; ScriptName="InstallPowerShell.ps1";        Sha256=$null; Group="Main";
    Tooltip="Shows a WPF picker to install or upgrade PowerShell versions.`n`nSteps:`n  1. Detects installed Windows PowerShell 5.x and PowerShell 7.x versions`n  2. Fetches latest PS7 release from GitHub API (PowerShell/PowerShell)`n  3. Shows WPF dialog with version cards and status badges (Up to date / Update available)`n  4. Installs selected via winget (primary) or MSI download (fallback)`n`nNote: PS5 is informational only - it is updated via Windows Update / WMF." },
  [pscustomobject]@{ Id="1.9";  Title="Upgrade all apps via Winget (excluding pinned)"; Type="Remote"; ScriptName="UpgradeWingetApps.ps1";        Sha256=$null; Group="Main";
    Tooltip="Scans for available application upgrades via winget and shows a WPF picker.`n`nSteps:`n  1. Runs 'winget upgrade --include-unknown' (via cmd redirect to avoid VT100 issues)`n  2. Parses output into a searchable/filterable WPF ListView`n  3. All upgrades are pre-selected; uncheck to skip individual apps`n  4. Installs selected apps via 'winget upgrade --id' with --silent flag`n`n- Requires winget to be installed (use 'Install Winget' if missing)`n- Pinned packages are not shown (winget respects pin rules)" },
  ...
  [pscustomobject]@{ Id="2.1"; Title="PS Module Manager";                               Type="Remote"; ScriptName="https://karanik.gr/scripts/powershell/PSModuleManager/PSModuleManager.ps1"; Sha256=$null; Group="PowerShell Tools";
    Tooltip="WPF GUI for managing PowerShell modules across System, Graph, and Azure categories.`n`nFeatures:`n  - View installed vs available modules from a curated catalog`n  - Install, update, or remove individual modules or by category`n  - Supports AllUsers scope installation`n  - Categories: System, Microsoft Graph SDK, Azure Az modules`n  - Thread-safe logging via ConcurrentQueue`n`nExamples of managed modules:`n  PSWindowsUpdate, ExchangeOnlineManagement, PnP.PowerShell`n  Microsoft.Graph.*, Az.*, MicrosoftTeams" },
  ...
  [pscustomobject]@{ Id="3.1"; Title="Clean all Temporary files (system & users)"; Type="Remote"; ScriptName="CleanTempFiles.ps1";              Sha256=$null; Group="Cleaning Tools";
    Tooltip="Deletes temporary files from system and all user profiles.`n`nLocations cleaned:`n  - C:\Windows\Temp\*`n  - C:\Users\*\AppData\Local\Temp\*`n  - %TEMP% (current user)`n`n- Skips files in use (no errors on locked files)`n- Reports total freed disk space" },
  [pscustomobject]@{ Id="3.2"; Title="Clear Teams cache only";                     Type="Remote"; ScriptName="ClearTeamsCache.ps1";              Sha256=$null; Group="Cleaning Tools";
    Tooltip="Clears Microsoft Teams cache folders for all user profiles.`n`n- Stops Teams processes before clearing`n- Removes cache files from AppData\Roaming\Microsoft\Teams (blobs, Cache, databases, etc.)`n- Does NOT uninstall Teams`n`nUseful for: Teams login issues, slow startup, UI glitches, stale avatar/channel data" },
  [pscustomobject]@{ Id="3.3"; Title="Teams Full clean + reinstall";   Type="Standalone"; ScriptName="TeamsCleanupAndInstall.ps1";   Sha256=$null; Group="Cleaning Tools";
    Tooltip="Completely removes Microsoft Teams and reinstalls the latest version.`n`n- Kills all Teams-related processes`n- Uninstalls Teams (Machine-Wide Installer + per-user AppData)`n- Clears all Teams cache and profile folders`n- Downloads and installs the latest Teams client from Microsoft CDN`n`nOpens as a separate window.`nUseful for: persistent Teams corruption, failed updates, login loops" },
  [pscustomobject]@{ Id="3.4"; Title="Cleanup SoftwareDistribution";               Type="Remote"; ScriptName="Cleanup-SoftwareDistribution.ps1"; Sha256=$null; Group="Cleaning Tools";
    Tooltip="Cleans only the SoftwareDistribution folder (targeted, lighter than Reset WU).`n`nSteps:`n  1. Stops WU-related services (wuauserv, bits, cryptsvc, msiserver, appidsvc)`n  2. Measures folder size before cleanup`n  3. Deletes contents of C:\Windows\SoftwareDistribution\ (preserves DataStore subfolder)`n  4. Removes stale SoftwareDistribution.old* / .bak* backup folders`n  5. Restarts services and reports freed disk space`n`nDoes NOT touch catroot2 or reset service permissions (use 'Reset Windows Update' for full reset)" },
  ...
  [pscustomobject]@{ Id="4.1"; Title="System Information (show + save full report)";       Type="Remote"; ScriptName="SystemInfo.ps1";                Sha256=$null; Group="Export Tools";
    Tooltip="Collects comprehensive system information and saves a full report.`n`nIncludes: OS version, hardware (CPU, RAM, disk), BIOS, network adapters,`ninstalled hotfixes, running services, startup programs, and environment variables.`nSaves report to Desktop as SystemInfo_<hostname>_<timestamp>.txt" },
  [pscustomobject]@{ Id="4.2"; Title="Export Installed Software list (CSV to Desktop)";    Type="Remote"; ScriptName="ExportInstalledSoftware.ps1";    Sha256=$null; Group="Export Tools";
    Tooltip="Exports a full list of installed software to CSV on the Desktop.`n`n- Queries both 32-bit and 64-bit registry uninstall keys`n- Includes: Name, Version, Publisher, InstallDate, InstallLocation`n- Output: InstalledSoftware_<hostname>_<timestamp>.csv" },
  [pscustomobject]@{ Id="4.3"; Title="Export Drivers (CSV to Desktop)";                    Type="Remote"; ScriptName="ExportDrivers.ps1";              Sha256=$null; Group="Export Tools";
    Tooltip="Exports all installed device drivers to a CSV file on the Desktop.`n`n- Uses Get-WindowsDriver or dism to enumerate driver packages`n- Includes: OriginalFileName, ProviderName, Version, Date, BootCritical`n- Output: Drivers_<hostname>_<timestamp>.csv" },
  [pscustomobject]@{ Id="4.4"; Title="Wi-Fi Profiles: Export to Desktop (XML, key=clear)"; Type="Remote"; ScriptName="ExportWiFiProfiles.ps1";         Sha256=$null; Group="Export Tools";
    Tooltip="Exports all saved Wi-Fi profiles to XML files on the Desktop.`n`n- Uses 'netsh wlan export profile key=clear' to include the plaintext Wi-Fi password`n- Each profile saved as a separate .xml file`n- Output folder: WiFiProfiles_<timestamp>\ on Desktop`n`nWarning: Exported XML files contain plaintext passwords. Handle securely." },
  [pscustomobject]@{ Id="4.5"; Title="Wi-Fi Profiles: Import from Desktop XML";            Type="Remote"; ScriptName="ImportWiFiProfiles.ps1";         Sha256=$null; Group="Export Tools";
    Tooltip="Imports Wi-Fi profiles from XML files located on the Desktop.`n`n- Scans Desktop for .xml Wi-Fi profile files`n- Uses 'netsh wlan add profile' to import each one`n- Reports success/failure per profile`n`nUse together with 'Wi-Fi Profiles: Export' to migrate profiles between machines." },
  [pscustomobject]@{ Id="4.6"; Title="Scheduled Tasks Audit (CSV to Desktop + console)";   Type="Remote"; ScriptName="ExportScheduledTasks.ps1";       Sha256=$null; Group="Export Tools";
    Tooltip="Audits all Scheduled Tasks and exports results to CSV and console.`n`n- Enumerates tasks from all folders recursively`n- Includes: TaskName, Path, State, LastRunTime, NextRunTime, Actions, Author`n- Highlights disabled and recently failed tasks`n- Output: ScheduledTasks_<hostname>_<timestamp>.csv on Desktop" },
  [pscustomobject]@{ Id="4.7"; Title="Windows Update History Summary (CSV + console)";     Type="Remote"; ScriptName="ExportWindowsUpdateHistory.ps1"; Sha256=$null; Group="Export Tools";
    Tooltip="Exports Windows Update installation history to CSV and displays a summary.`n`n- Queries WUA COM API for update history`n- Includes: Title, KB, Date, Result (Succeeded/Failed), Operation type`n- Shows failed updates highlighted in console`n- Output: WUHistory_<hostname>_<timestamp>.csv on Desktop" },
  [pscustomobject]@{ Id="4.8"; Title="Battery Report";                                     Type="Remote"; ScriptName="BatteryReport.ps1";              Sha256=$null; Group="Export Tools";
    Tooltip="Generates a Windows battery report using powercfg /batteryreport.`n`n- Saves full HTML report to Desktop`n- Opens the report automatically in the default browser`n- Includes: battery capacity history, charge/drain cycles, recent usage`n`nNote: Only useful on laptops and tablets with a battery." },

  # ── 5. Fix Tools ───────────────────────────────────────────────────────────
  [pscustomobject]@{ Id="5.1"; Title="Reset Print Spooler";                          Type="Remote"; ScriptName="ResetPrintSpooler.ps1";         Sha256=$null; Group="Fix Tools";
    Tooltip="Resets the Windows Print Spooler service to fix stuck print jobs or spooler crashes.`n`n- Stops the Spooler service`n- Clears all pending print jobs from C:\Windows\System32\spool\PRINTERS\`n- Restarts the Spooler service`n`nUseful for: printers showing 'Deleting' jobs, offline printers, spooler not starting" },
  [pscustomobject]@{ Id="5.2"; Title="Network Reset";                                Type="Remote"; ScriptName="NetworkReset.ps1";               Sha256=$null; Group="Fix Tools";
    Tooltip="Performs a full network stack reset to fix persistent connectivity issues.`n`n- Resets Winsock catalog (netsh winsock reset)`n- Resets TCP/IP stack (netsh int ip reset)`n- Resets Windows Firewall policies (netsh advfirewall reset)`n- Flushes DNS cache (ipconfig /flushdns)`n- Releases and renews IP address`n`nRESTART REQUIRED after completion." },
  [pscustomobject]@{ Id="5.3"; Title="Wi-Fi Reset";                                  Type="Remote"; ScriptName="WiFiReset.ps1";                  Sha256=$null; Group="Fix Tools";
    Tooltip="Resets only the Wi-Fi adapter and related stack components.`n`n- Disables and re-enables the Wi-Fi adapter`n- Flushes DNS cache`n- Resets Winsock (winsock reset)`n- Optionally removes and re-adds saved Wi-Fi profiles`n`nUseful for: Wi-Fi not connecting, limited connectivity, adapter stuck in error state" },
  [pscustomobject]@{ Id="5.4"; Title="Set Classic Right-Click Menu (Win11)";         Type="Remote"; ScriptName="SetClassicContextMenu.ps1";     Sha256=$null; Group="Fix Tools";
    Tooltip="Restores the classic (Windows 10-style) right-click context menu in Windows 11.`n`n- Sets registry key: HKCU\Software\Classes\CLSID\{86ca1aa0...}\InprocServer32 = (empty)`n- Explorer is restarted to apply the change immediately`n`nResult: Right-click shows full menu directly without needing 'Show more options'." },
  [pscustomobject]@{ Id="5.5"; Title="Restore Default Right-Click Menu (Win11)";     Type="Remote"; ScriptName="RestoreDefaultContextMenu.ps1"; Sha256=$null; Group="Fix Tools";
    Tooltip="Restores the default Windows 11 right-click context menu (collapsed with 'Show more options').`n`n- Removes the classic menu override registry key`n- Explorer is restarted to apply the change immediately`n`nUse to undo the 'Set Classic Right-Click Menu' change." },
  [pscustomobject]@{ Id="5.6"; Title="Disable Hibernation";                          Type="Remote"; ScriptName="DisableHibernation.ps1";        Sha256=$null; Group="Fix Tools";
    Tooltip="Disables Windows Hibernation and removes the hiberfil.sys file.`n`n- Runs: powercfg /hibernate off`n- Deletes C:\hiberfil.sys (frees disk space - typically 40-75% of installed RAM)`n`nUseful for: reclaiming disk space on SSD, eliminating hibernation-related wake issues." },
  [pscustomobject]@{ Id="5.7"; Title="Enable Hibernation";                           Type="Remote"; ScriptName="EnableHibernation.ps1";         Sha256=$null; Group="Fix Tools";
    Tooltip="Enables Windows Hibernation and recreates hiberfil.sys.`n`n- Runs: powercfg /hibernate on`n- hiberfil.sys will be recreated (size = ~40-75% of installed RAM)`n`nRequired for: Hybrid Sleep, Fast Startup, and hibernate power button action." },
  [pscustomobject]@{ Id="5.8";  Title="WebView2 Repair / Install";                    Type="Remote"; ScriptName="WebView2_Repair_Install.ps1";     Sha256=$null; Group="Fix Tools";
    Tooltip="Checks if Microsoft Edge WebView2 Runtime is installed.`nDownloads and installs it if missing or forced via -ForceReinstall.`nKills msedgewebview2.exe processes before install (unless -SkipKillProcesses).`n`nUseful for: Outlook Modern Auth popup crashes, WebView2-related app failures." },
  [pscustomobject]@{ Id="5.9";  Title="WebView2 / Outlook Auth Repair (Advanced)";   Type="Remote"; ScriptName="WebView2_Repair_Install_v3.ps1";    Sha256=$null; Group="Fix Tools";
    Tooltip="Advanced repair tool for Microsoft 365 / Exchange Online Outlook authentication issues.`n`nActions performed:`n  - Installs WebView2 if missing`n  - Cleans AAD BrokerPlugin / IdentityCache / OneAuth caches`n  - Resets stored Office / ADAL / SSO credentials`n  - Collects AAD Broker registration diagnostics`n  - Detects join state (Azure AD / Entra / Hybrid / Domain)`n  - Exports AAD, Office, WAM, AppX event logs`n  - Tests network connectivity to Microsoft 365 auth endpoints`n  - Triggers Office Click-to-Run repair (best effort)`n  - Exports full diagnostics package`n`nUseful for: Outlook sign-in window opens and closes, Modern Auth popup crashes, WAM/token corruption." },
  [pscustomobject]@{ Id="5.10"; Title="Remove iCloud (All Users)";                   Type="Remote"; ScriptName="Remove_iCloud_AllUsers.ps1";         Sha256=$null; Group="Fix Tools";
    Tooltip="Removes iCloud / Apple components from ALL local user profiles.`n`nAlways performed:`n  - Kills: Outlook, iCloud, iCloudServices, AppleOutlookDAVConfig, msedgewebview2`n  - Deletes Apple/iCloud folders under C:\Users\*\AppData (Local + Roaming)`n  - Deletes C:\ProgramData\Apple*`n`nNot performed by default (requires script modification):`n  - Appx package removal (-RemovePackages)`n  - Program Files removal (-RemoveProgramFiles)`n  - Registry cleanup (-RemoveRegistry)`n  - Scheduled Tasks removal (-RemoveScheduledTasks)`n  - Services removal (-RemoveServices)`n`nRESTART REQUIRED after completion.`nRecommended for troubleshooting environments only." },
  ...
  [pscustomobject]@{ Id="6.1"; Title="Reset RDS Licensing Grace Period (Windows Server)"; Type="Remote";  ScriptName="RDS-GracePeriod-Manager.ps1"; Sha256=$null; Group="Extra Tools";
    Tooltip="Resets the Remote Desktop Services (RDS) licensing grace period on Windows Server.`n`n- Removes the grace period registry key that tracks the 120/180-day trial`n- Forces RDS to restart the grace period counter`n- Useful when RDS Licensing server is not yet configured`n`nTarget: HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server\RCM\GracePeriod`nRESTART REQUIRED after completion.`n`nNote: Use only in legitimate lab/dev environments or while configuring proper RDS CALs." },
  [pscustomobject]@{ Id="6.2"; Title="Microsoft Activation Scripts (MAS)";                Type="Inline";  Command="irm https://get.activated.win | iex"; Sha256=$null; Group="Extra Tools";
    Tooltip="Runs Microsoft Activation Scripts (MAS) by massgravel.`n`nSource: https://get.activated.win`nGitHub: https://github.com/massgravel/Microsoft-Activation-Scripts`n`nProvides activation options for Windows and Office using HWID, KMS38, or Online KMS methods.`nOpens an interactive menu in a new console window." },
  [pscustomobject]@{ Id="6.3"; Title="WinScript";                                         Type="Inline";  Command='irm "https://winscript.cc/irm" | iex'; Sha256=$null; Group="Extra Tools";
    Tooltip="Runs WinScript - a Windows tweaks and configuration utility.`n`nSource: https://winscript.cc`n`nProvides an interactive menu for privacy tweaks, debloating, performance settings,`nand other Windows customizations. Opens in a new console window." },
  [pscustomobject]@{ Id="6.4"; Title="Chris Titus Tech's Windows Utility";                Type="Inline";  Command='irm "https://christitus.com/win" | iex'; Sha256=$null; Group="Extra Tools";
    Tooltip="Runs Chris Titus Tech's WinUtil - a comprehensive Windows configuration tool.`n`nSource: https://christitus.com/win`nGitHub: https://github.com/ChrisTitusTech/winutil`n`nFeatures: Install/uninstall software, apply tweaks, repair Windows, configure settings.`nOpens a WPF GUI in a new window." },
  [pscustomobject]@{ Id="6.5"; Title="WinDiag-AI";                                        Type="Standalone"; ScriptName="https://karanik.gr/scripts/powershell/WinDiag-AI/WinDiag-AI.ps1"; Sha256=$null; Group="Extra Tools";
    Tooltip="Windows Diagnostics with AI Analysis (WPF GUI)`nby Nikolaos Karanikolas`n`nCollects system diagnostics and sends them to a local AI (Ollama) for analysis.`n`nFeatures:`n  - SMART disk details`n  - PDF export`n  - Custom checks`n  - Driver check`n  - Dark / Light theme`n  - Expanded settings and tooltips`n`nOpens as a separate window. Run as Administrator for full diagnostics." },
  [pscustomobject]@{ Id="6.6"; Title="AD Manager";                                        Type="Standalone"; ScriptName="https://karanik.gr/scripts/powershell/AD_Manager/AD_Manager.ps1"; Sha256=$null; Group="Extra Tools";
    Tooltip="Active Directory Manager (WPF GUI)`nby Nikolaos Karanikolas`n`nA graphical tool for common Active Directory administration tasks.`n`nOpens as a separate window. Requires Active Directory module (RSAT) and domain connectivity." },
  [pscustomobject]@{ Id="6.7"; Title="Network Scanner";                                   Type="Standalone"; ScriptName="https://karanik.gr/scripts/powershell/netscan/netscan.ps1"; Sha256=$null; Group="Extra Tools";
    Tooltip="netscan is a Windows GUI network scanner built in PowerShell + WinForms. It scans subnets for live hosts, identifies their MAC addresses and NIC vendors, detects unauthorized (rogue) DHCP servers." },
